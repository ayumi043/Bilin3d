Thingiloader = function(event) {
  ...
  this.loadSTL = function(url) {
    ...
    workerFacadeMessage({'status':'message', 'content':''});  
  ...
  this.loadOBJ = function(url) {
    workerFacadeMessage({'status':'message', 'content':' <img src="catalog/view/theme/3d/image/loading.gif" />'});  
    var file = this.load_binary_resource(url);
  ...
  this.loadPLY = function(url) {
    workerFacadeMessage({'status':'message', 'content':'asdfds '});  
