    ...
    worker.onmessage = function(event) {
      if (event.data.status == "complete") {
       ...
       // progressBar.innerHTML = '';
		...
		document.getElementById('text_loading_after').style.display = 'none';
        scope.setRotation(false);
        scope.setRotation(true);
        log("finished loading " + geometry.faces.length + " faces.");
        scope.centerCamera();
        ...
        document.getElementById('processBar2Span').style.height = event.data.content;
        // log(event.data.content);
      } else if (event.data.status == "message") {
        progressBar.style.display = 'block';
		progressBar2.style.display = 'block';
        progressBar2.innerHTML = '<span id="processBar2Span"></span>';
		document.getElementById('text_loading_after').style.display = 'block';
        log(event.data.content);
...
var STLGeometry = function(stlArray) {
  ...
  halfx = (scope.max_x + scope.min_x)/2;
  halfy = (scope.max_y + scope.min_y)/2;
  halfz = (scope.max_z + scope.min_z)/2;
  
  for (var v = 0, vl = scope.vertices.length; v < vl; v ++) {
	  scope.vertices[v].position.x = scope.vertices[v].position.x-halfx; 
	  scope.vertices[v].position.y = scope.vertices[v].position.y-halfy; 
	  scope.vertices[v].position.z = scope.vertices[v].position.z-halfz; 
  }
